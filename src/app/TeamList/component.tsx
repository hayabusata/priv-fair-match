...
export const TeamList = () => {
  ...
  const win = (e: any) => {
    const winners =
      "alpha" === e.currentTarget.getAttribute("data-winner")
        ? alphaList.slice(-1)[0]
        : betaList.slice(-1)[0];
    ...
    const newAllPlayers = allPlayers.map((player) => {
      return {
        ...player,
        win:
          winners.findIndex((winner) => winner.name === player.name) > -1
            ? player.win + 1
            : player.win,
      };
    });
                  ...
                  {alpha.map((player, key) => {
                    return (
                      <th key={key}>
                        ...
                        <span
                          className={`${player.win === mostWins ? styles.mostWins : ""} ${styles.win}`}
                        >
                          ({player.win})
                        </span>
                      </th>
